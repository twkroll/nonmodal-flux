...
constant linear dynamics, its positive-input-metric whitening, signed terminal
extrema and modes, and reconstruction of extremal physical input coordinates.
"""
...
def terminal_signed_extremal_inputs(
    problem: TransportProblem,
    T: Real,
) -> tuple[Array, Array, Array, Array]:
    """Return terminal signed extrema and optimizers in physical input coordinates.

    With ``Rin = L L^H`` and whitened coordinates ``v = L^H u``, the physical
    input vector is recovered from

    ``L^H u = v``.

    The solve is triangular; no inverse of ``L`` or ``Rin`` is formed. Each
    reconstructed input is then normalized so that
    ...
    (jax.Array, jax.Array, jax.Array, jax.Array)
        ...
        ``u_max`` are expressed in the original admissible input coordinates and
        have unit ``Rin`` cost.
    ...
    lambda_min, v_min, lambda_max, v_max = terminal_signed_extremal_modes(problem, T)
    rin = jnp.asarray(problem.Rin)
    lower = jnp.linalg.cholesky(rin)
    upper = lower.conj().T
