"""Tests for extremal modes and reconstructed inputs of accumulated transport."""
...
def _complex_metric_problem() -> TransportProblem:
    return TransportProblem(
        A=np.array(
...
def test_accumulated_extremal_mode_rayleigh_quotients_equal_extrema() -> None:
    problem = _complex_metric_problem()
    horizon = 0.39
...
def test_accumulated_extremal_inputs_have_unit_cost_and_satisfy_generalized_eigenproblem() -> None:
    problem = _nontrivial_metric_problem()
    horizon = 0.43

    lambda_min, u_min, lambda_max, u_max = accumulated_signed_extremal_inputs(
        problem, horizon
    )
    operator = np.asarray(accumulated_input_transport_operator(problem, horizon))
    rin = problem.Rin

    for eigenvalue, vector in ((lambda_min, u_min), (lambda_max, u_max)):
        lam = float(np.asarray(eigenvalue))
        u = np.asarray(vector)
        cost = np.vdot(u, rin @ u)

        np.testing.assert_allclose(cost, 1.0, rtol=1.0e-12, atol=1.0e-12)
...
def test_accumulated_extremal_input_reconstruction_supports_complex_input_metric() -> None:
    problem = _complex_metric_problem()
    horizon = 0.37
    ...
    for eigenvalue, vector in ((lambda_min, u_min), (lambda_max, u_max)):
        ...
        u = np.asarray(vector)
        cost = np.vdot(u, rin @ u)
        accumulated_value = np.vdot(u, operator @ u)
